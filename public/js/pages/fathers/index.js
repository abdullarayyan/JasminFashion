$(document).ready(() => {
    ...
    renderFathers = (response) => {
        ...
        if (response && response.data.length > 0) {
            $("#fathersTable tbody tr").remove();

            const fathers = response.data;
            let table = document.getElementById('fathersTable');
            ...
            fathers.forEach(father => {
                ...
                td0.appendChild(document.createTextNode(father.full_name));
                new_row.appendChild(td0);

                let td1 = document.createElement('td');
                td1.appendChild(document.createTextNode(father.idc));
                new_row.appendChild(td1);

                let td2 = document.createElement('td');
                td2.appendChild(document.createTextNode(father.nationality));
                new_row.appendChild(td2);

                let td3 = document.createElement('td');
                td3.setAttribute('class', 'yatem_name');
                let link = document.createElement('a');
                link.setAttribute('href', `/edit-yatem-form/${father.yatem_id}`);
                let linkText = document.createTextNode(father.yatem_name);
                link.appendChild(linkText);
                td3.appendChild(link);
                new_row.appendChild(td3);

                let td4 = document.createElement('td');
                td4.appendChild(document.createTextNode(father.yatem_idc));
            ...
            alert.appendChild(document.createTextNode('لا يوجد آباء'));
    ...
    getFathers = (allPages = currentPage) => {
        document.getElementById('hidden_filter').value = JSON.stringify(filter);

        $.ajax({
            type: 'GET',
            url: `/fathers/fathers-list/${pageSize}?page=${allPages}&filter=${JSON.stringify(filter)}`,
            success: (response) => {
                renderFathers(response);
            },
            error: (error) => {
